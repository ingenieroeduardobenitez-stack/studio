...
const formSchema = z.object({
  ...
  baptismParish: z.string().optional().nullable(),
  baptismBook: z.string().optional().nullable(),
  baptismFolio: z.string().optional().nullable(),
  baptismCertificatePhotoUrl: z.string().optional().nullable(),
})
  ...
  const handleLookupCi = async (ciValue: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        toast({ title: "Cédula no encontrada", description: "Por favor, completa los campos manualmente." });
      }
    } catch (error: any) {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleRegistration)}>
          <CardHeader className="bg-primary text-white p-8">
            ...
            <CardDescription className="text-white/70">Completa todos los campos obligatorios (*)</CardDescription>
          </CardHeader>
          <CardContent className="p-8 space-y-12">
            <div className="space-y-6">
              <div className="flex items-center gap-3 border-b pb-2"><UserPlus className="h-5 w-5 text-primary" /><h3 className="font-bold uppercase text-xs tracking-widest text-slate-500">1. Identidad</h3></div>
              <div className="grid gap-6 md:grid-cols-2">
                <div className="md:col-span-2 flex flex-col items-center py-4">
                  <Label className="font-bold text-slate-700 mb-4">Foto de Perfil *</Label>
                  <div className="relative group">
                    ...
                    <div className="absolute -bottom-2 -right-2 flex gap-2">
                      <Button type="button" size="icon" className="h-10 w-10 rounded-full bg-primary" onClick={() => startCamera("photoUrl")}><Camera className="h-4 w-4" /></Button>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 border-b pb-2"><Heart className="h-5 w-5 text-pink-500" /><h3 className="font-bold uppercase text-xs tracking-widest text-slate-500">2. Familia</h3></div>
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4 p-6 bg-slate-50 rounded-[2rem] border">
                  <p className="text-[10px] font-black uppercase text-slate-400">Madre</p>
                  <FormField control={form.control} name="motherName" render={({ field }) => (<FormItem><FormLabel className="text-xs">Nombre</FormLabel><FormControl><Input {...field} value={field.value || ""} className="h-10 rounded-lg bg-white uppercase font-medium" /></FormControl></FormItem>)} />
                  <FormField control={form.control} name="motherPhone" render={({ field }) => (<FormItem><FormLabel className="text-xs">Celular</FormLabel><FormControl><Input {...field} value={field.value || ""} className="h-10 rounded-lg bg-white" /></FormControl></FormItem>)} />
                </div>
                <div className="space-y-4 p-6 bg-slate-50 rounded-[2rem] border">
                  <p className="text-[10px] font-black uppercase text-slate-400">Padre</p>
                  <FormField control={form.control} name="fatherName" render={({ field }) => (<FormItem><FormLabel className="text-xs">Nombre</FormLabel><FormControl><Input {...field} value={field.value || ""} className="h-10 rounded-lg bg-white uppercase font-medium" /></FormControl></FormItem>)} />
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 border-b pb-2"><Church className="h-5 w-5 text-primary" /><h3 className="font-bold uppercase text-xs tracking-widest text-slate-500">3. Nivel</h3></div>
              <div className="grid gap-6 md:grid-cols-2">
                <FormField control={form.control} name="catechesisYear" render={({ field }) => (<FormItem><FormLabel className="font-bold">Año *</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className="h-12 rounded-xl"><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="PRIMER_AÑO">PRIMER AÑO</SelectItem><SelectItem value="SEGUNDO_AÑO">SEGUNDO AÑO</SelectItem><SelectItem value="ADULTOS">ADULTOS</SelectItem></SelectContent></Select></FormItem>)} />
                <FormField control={form.control} name="attendanceDay" render={({ field }) => (<FormItem><FormLabel className="font-bold">Día *</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className="h-12 rounded-xl"><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="SABADO">SÁBADO (15:30 hs)</SelectItem><SelectItem value="DOMINGO">DOMINGO (08:00 hs)</SelectItem></SelectContent></Select></FormItem>)} />
              </div>
            </div>

            <div className="space-y-6">
              <div className="flex items-center gap-3 border-b pb-2"><FileText className="h-5 w-5 text-orange-500" /><h3 className="font-bold uppercase text-xs tracking-widest text-slate-500">4. Sacramentos</h3></div>
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4">
                  <FormField control={form.control} name="hasBaptism" render={({ field }) => (<FormItem className="flex flex-row items-start space-x-3 p-4 border rounded-xl bg-slate-50/50"><FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl><div className="leading-none"><FormLabel className="font-bold">¿Bautizado?</FormLabel></div></FormItem>)} />
                  <FormField control={form.control} name="hasFirstCommunion" render={({ field }) => (<FormItem className="flex flex-row items-start space-x-3 p-4 border rounded-xl bg-slate-50/50"><FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl><div className="leading-none"><FormLabel className="font-bold">¿Primera Comunión?</FormLabel></div></FormItem>)} />
                </div>
                {hasBaptism && (
                  <div className="space-y-4 p-6 bg-orange-50/30 rounded-[2rem] border border-orange-100 animate-in zoom-in-95">
                    <p className="text-[10px] font-black uppercase text-orange-600">Datos Bautismo</p>
                    <FormField control={form.control} name="baptismParish" render={({ field }) => (<FormItem><FormLabel className="text-xs">Parroquia</FormLabel><FormControl><Input {...field} value={field.value || ""} className="h-10 rounded-lg bg-white" /></FormControl></FormItem>)} />
                    <div className="grid grid-cols-2 gap-3"><FormField control={form.control} name="baptismBook" render={({ field }) => (<FormItem><FormLabel className="text-xs">Libro</FormLabel><FormControl><Input {...field} value={field.value || ""} className="h-10 rounded-lg bg-white" /></FormControl></FormItem>)} /><FormField control={form.control} name="baptismFolio" render={({ field }) => (<FormItem><FormLabel className="text-xs">Folio</FormLabel><FormControl><Input {...field} value={field.value || ""} className="h-10 rounded-lg bg-white" /></FormControl></FormItem>)} /></div>
                    <div className="pt-2"><Label className="text-xs">Foto Certificado</Label><div className="flex gap-2 mt-2"><Button type="button" variant="outline" className="flex-1 h-10" onClick={() => startCamera("baptismCertificatePhotoUrl")}><Camera className="h-4 w-4 mr-2" /> Cámara</Button><Button type="button" variant="outline" className="flex-1 h-10" onClick={() => { const input = document.createElement('input'); input.type = 'file'; input.accept = 'image/*'; input.onchange = (e: any) => handleFileUpload(e, "baptismCertificatePhotoUrl"); input.click(); }}><ImageIcon className="h-4 w-4 mr-2" /> Galería</Button></div>{watch("baptismCertificatePhotoUrl") && <div className="relative h-24 w-full rounded-xl overflow-hidden mt-2 border"><img src={watch("baptismCertificatePhotoUrl") || ""} className="w-full h-full object-cover" /><button type="button" className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full" onClick={() => setValue("baptismCertificatePhotoUrl", null)}><X className="h-3 w-3" /></button></div>}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 border-b pb-2"><Wallet className="h-5 w-5 text-primary" /><h3 className="font-bold uppercase text-xs tracking-widest text-slate-500">5. Pago</h3></div>
              <div className="grid gap-6 md:grid-cols-2 items-center">
                <div className="p-6 bg-slate-50 rounded-[2rem] border border-dashed text-center"><p className="text-[10px] font-black uppercase text-slate-400">Arancel Oficial:</p><p className="text-4xl font-black text-primary">{establishedLimit.toLocaleString('es-PY')} Gs.</p></div>
              ...
              {paymentMethod === "TRANSFERENCIA" && (
                <div className="p-8 bg-blue-50/50 rounded-[2.5rem] border border-blue-100 space-y-4">
                  <div className="flex items-center gap-3"><Info className="h-5 w-5 text-blue-600" /><p className="text-sm font-bold text-blue-900">Datos de Transferencia</p></div>
                  <div className="bg-white p-6 rounded-3xl border shadow-sm grid grid-cols-2 gap-4"><div className="col-span-2 bg-blue-50 p-3 rounded-xl text-center"><p className="text-[9px] font-bold text-blue-600 uppercase">Alias SIPAP</p><p className="text-lg font-black text-blue-700">{costs?.alias || "parroquia.ps"}</p></div><div><p className="text-[9px] font-bold text-slate-500 uppercase">Banco</p><p className="text-sm font-black text-slate-900">{costs?.bankName || "ueno bank"}</p></div><div><p className="text-[9px] font-bold text-slate-500 uppercase">Titular</p><p className="text-sm font-black text-slate-900">{costs?.accountOwner || "Santuario NSPS"}</p></div></div>
                  <div className="space-y-3 pt-4"><Label className="font-bold text-blue-900">Foto Comprobante *</Label><div className="grid grid-cols-2 gap-3"><Button type="button" variant="outline" className="h-12" onClick={() => startCamera("paymentProofUrl")}><Camera className="h-4 w-4 mr-2" /> Cámara</Button><Button type="button" variant="outline" className="h-12" onClick={() => { const i = document.createElement('input'); i.type = 'file'; i.accept = 'image/*'; i.onchange = (e: any) => handleFileUpload(e, "paymentProofUrl"); i.click(); }}><ImageIcon className="h-4 w-4 mr-2" /> Galería</Button></div>{watch("paymentProofUrl") && <div className="relative h-48 w-full rounded-2xl overflow-hidden mt-2 border-2 border-blue-200"><img src={watch("paymentProofUrl") || ""} className="w-full h-full object-cover" /><button type="button" className="absolute top-2 right-2 p-1.5 bg-red-500 text-white rounded-full" onClick={() => setValue("paymentProofUrl", null)}><X className="h-4 w-4" /></button></div>}</div>
                </div>
              )}
            </div>
          </CardContent>
          <CardFooter className="bg-slate-50 p-10"><Button type="submit" disabled={loading} className="w-full h-20 bg-primary text-white rounded-3xl text-2xl font-black shadow-2xl active:scale-95 transition-all">{loading ? <Loader2 className="animate-spin h-8 w-8 mr-3" /> : <><Plus className="h-8 w-8 mr-3" /> ENVIAR MI INSCRIPCIÓN</>}</Button></CardFooter>
        </form>
      </Form>
      <Dialog open={showCamera} onOpenChange={(open) => { if(!open && currentStream) currentStream.getTracks().forEach(t => t.stop()); setShowCamera(open); }}><DialogContent className="sm:max-w-[500px] p-0 overflow-hidden rounded-[2.5rem] border-none shadow-2xl"><DialogHeader className="p-6 bg-primary text-white shrink-0"><DialogTitle className="text-center uppercase font-black text-sm">Capturar Fotografía</DialogTitle></DialogHeader><div className="relative bg-black aspect-square flex items-center justify-center overflow-hidden"><video ref={onVideoRef} autoPlay playsInline muted className="w-full h-full object-cover" /><canvas ref={canvasRef} className="hidden" /></div><DialogFooter className="p-6 bg-slate-50 border-t flex flex-col gap-4">{devices.length > 1 && (<Select value={selectedDeviceId} onValueChange={(val) => { setSelectedDeviceId(val); startCamera(cameraTarget, val); }}><SelectTrigger className="h-10 rounded-xl bg-white"><SelectValue placeholder="Cambiar Cámara" /></SelectTrigger><SelectContent>{devices.map((d) => (<SelectItem key={d.deviceId} value={d.deviceId}>{d.label || `Cámara ${d.deviceId.slice(0,5)}`}</SelectItem>))}</SelectContent></Select>)}<div className="flex gap-3"><Button type="button" variant="outline" className="flex-1 h-14 rounded-2xl font-black text-xs uppercase" onClick={() => { if(currentStream) currentStream.getTracks().forEach(t=>t.stop()); setShowCamera(false); }}>CANCELAR</Button><Button type="button" className="flex-1 h-14 bg-primary text-white font-black text-xs uppercase shadow-xl" onClick={takePhoto}>CAPTURAR AHORA</Button></div></DialogFooter></DialogContent></Dialog>
    </Card>
  );
}
