  ...
  const handleMarkAttendance = async (student: any, status: "PRESENTE" | "AUSENTE") => {
    ...
    try {
      ...
      await addDoc(collection(db, "audit_logs"), {
        ...
        action: "Control de Asistencia",
        module: "asistencia",
        details: `Se marcó a ${student.fullName} como ${status} para la fecha ${attendanceDate}`,
        timestamp: serverTimestamp()
      })
  ...
  const handleJustifyAttendance = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await addDoc(collection(db, "audit_logs"), {
        userId: currentUser?.uid || "unknown",
        userName: catechistName,
        action: "Justificación de Falta",
        module: "asistencia",
        details: `Se justificó la inasistencia de ${selectedStudent.fullName} (${attendanceDate}). Motivo: ${notes}`,
        timestamp: serverTimestamp()
      })

      toast({ title: "Ausencia justificada" })
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border shadow-sm">
            ...
            <Input type="date" className="border-none bg-transparent p-0 h-auto font-bold text-sm w-[130px]" value={attendanceDate} onChange={(e) => setAttendanceDate(e.target.value)} />
          </div>
        </div>
      </div>
      <div className="grid gap-6 md:grid-cols-4">
        <div className="md:col-span-1 space-y-6">
          <Card className="border-none shadow-xl bg-white overflow-hidden">
            <CardHeader className="bg-slate-50/50 border-b"><CardTitle className="text-sm flex items-center gap-2"><Filter className="h-4 w-4 text-primary" /> Filtrar Lista</CardTitle></CardHeader>
            <CardContent className="p-6 space-y-4">
              <div className="space-y-2">
                ...
                <Select value={selectedGroupId} onValueChange={setSelectedGroupId}>
                  ...
                  <SelectContent><SelectItem value="all">Todos los grupos</SelectItem>{groups?.map(g => <SelectItem key={g.id} value={g.id}>{g.name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-bold text-slate-400 uppercase">Buscador</Label>
                <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><Input placeholder="Nombre o C.I..." className="pl-9 h-11 rounded-xl bg-slate-50" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-none shadow-xl bg-primary text-white overflow-hidden relative">
            ...
            <CardContent><div className="text-3xl font-black">{filteredStudents.length}</div><p className="text-[10px] text-white/70 uppercase font-bold tracking-widest">Confirmandos en lista</p></CardContent>
          </Card>
        </div>
          ...
          <Card className="border-none shadow-xl overflow-hidden bg-white">
            <CardContent className="p-0">
              {loadingRegs ? (
                ...
                <div className="py-24 text-center"><User className="h-12 w-12 text-slate-200 mx-auto mb-4" /><p className="text-slate-500 font-medium">No se encontraron confirmandos.</p></div>
              ) : (
                <Table>
                  <TableHeader className="bg-slate-50/50"><TableRow><TableHead className="font-bold pl-8">Confirmando</TableHead><TableHead className="font-bold text-center">Inasistencias</TableHead><TableHead className="font-bold text-center">Estado Hoy</TableHead><TableHead className="text-right font-bold pr-8">Acciones</TableHead></TableRow></TableHeader>
                  <TableBody>
      ...
      <Dialog open={isJustifyDialogOpen} onOpenChange={setIsJustifyDialogOpen}>
        <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden border-none shadow-2xl">
          <DialogHeader className="p-6 bg-primary text-white shrink-0"><DialogTitle>Justificar Inasistencia</DialogTitle><DialogDescription className="text-white/80">Confirmando: {selectedStudent?.fullName}</DialogDescription></DialogHeader>
          <form onSubmit={handleJustifyAttendance} className="flex flex-col">
            <div className="p-6 space-y-6">
              <div className="space-y-3"><Label className="font-bold text-slate-700">Observaciones / Motivo</Label><Textarea name="notes" placeholder="Ej. Justificado por enfermedad." className="rounded-xl min-h-[100px] bg-slate-50 border-slate-200" required /></div>
              <div className="space-y-3">
                <Label className="font-bold text-slate-700">Comprobante</Label>
                <div className={cn("border-2 border-dashed rounded-2xl h-48 flex flex-col items-center justify-center cursor-pointer overflow-hidden transition-all", justificationProof ? "border-green-500 bg-green-50" : "border-slate-300 bg-slate-50 hover:bg-slate-100")} onClick={() => proofInputRef.current?.click()}>
                  {justificationProof ? <img src={justificationProof} className="w-full h-full object-cover" /> : <><ImageIcon className="h-10 w-10 text-slate-300 mb-2" /><span className="text-xs font-bold text-slate-400 uppercase">Subir Foto</span></>}
                </div>
                <input type="file" ref={proofInputRef} className="hidden" accept="image/*" onChange={handleFileChange} />
              </div>
            </div>
            <DialogFooter className="p-6 bg-slate-50 border-t flex gap-3"><Button type="button" variant="outline" className="flex-1 h-12 rounded-xl" onClick={() => setIsJustifyDialogOpen(false)}>Cancelar</Button><Button type="submit" className="flex-1 h-12 rounded-xl bg-primary hover:bg-primary/90 font-bold shadow-lg" disabled={isSubmitting}>{isSubmitting ? <Loader2 className="animate-spin h-4 w-4" /> : "Guardar Justificación"}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    </div>
