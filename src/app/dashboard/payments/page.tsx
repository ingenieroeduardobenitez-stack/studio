    ...
    try {
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (doc) => {
          ...
          if (el) { el.style.transform = "none"; el.style.width = "800px"; el.style.maxWidth = "800px"; el.style.margin = "0"; el.style.padding = "40px"; }
        }
      });
      ...
      <Dialog open={isReceiptOpen} onOpenChange={setIsReceiptOpen}>
        <DialogContent className="sm:max-w-[800px] p-0 overflow-hidden border-none shadow-2xl bg-white rounded-xl">
          ...
          <div className="p-6 bg-white flex justify-center overflow-y-auto max-h-[85vh]">
            <div className="w-full max-w-[700px] bg-white text-slate-900 font-serif border-2 border-slate-900 p-8 space-y-10" id="receipt-content-official">
              <div className="grid grid-cols-3 gap-4 items-center mb-6"><div className="col-span-2 border-2 border-slate-900 p-4 min-h-[120px] flex items-center justify-center relative bg-white"><img src="/logo.png" alt="Logo Santuario" className="max-h-24 object-contain" /><div className="absolute top-1 right-2 text-[7px] font-black uppercase tracking-tighter text-slate-400 text-right leading-tight">Santuario Nacional<br/>Nuestra Señora del Perpetuo Socorro</div></div><div className="flex flex-col gap-2 h-full justify-between"><div className="border-2 border-slate-900 p-2 text-center bg-slate-50"><p className="text-[10px] font-black uppercase tracking-tighter">Gs.</p><p className="text-xl font-black">{paymentAmount.toLocaleString('es-PY')}</p></div><div className="border-2 border-slate-900 p-2 text-center bg-white"><p className="text-[8px] font-bold uppercase">Recibo N°</p><p className="text-xs font-black">{selectedReg?.receiptNumber || "PENDIENTE"}</p></div></div></div>
              <div className="text-center border-b-2 border-slate-900 pb-2 mb-4"><h1 className="text-3xl font-black italic tracking-tighter uppercase">RECIBO</h1></div>
              <div className="space-y-10 text-base"><div className="flex items-baseline gap-2 py-1"><span className="whitespace-nowrap font-bold shrink-0 tracking-wide">Recibí(mos) de:</span><div className="flex-1 border-b border-dotted border-slate-400 font-bold uppercase pb-1 px-2 leading-relaxed truncate">{selectedReg?.fullName}</div></div><div className="flex items-baseline gap-2 py-1"><span className="whitespace-nowrap font-bold shrink-0 tracking-wide">la cantidad de:</span><div className="flex-1 border-b border-dotted border-slate-400 pb-1 px-2 italic leading-relaxed">{paymentAmount.toLocaleString('es-PY')} Guaraníes</div></div><div className="space-y-3"><div className="flex items-baseline gap-2 py-1"><span className="whitespace-nowrap font-bold shrink-0 tracking-wide">en concepto de:</span><div className="flex-1 border-2 border-slate-900 px-4 py-2 font-bold text-xs bg-slate-50 uppercase leading-relaxed">{selectedEventId === 'inscripcion' ? 'Inscripción Catequesis de Confirmación' : (selectedEvent?.category || 'Evento Parroquial')} - {selectedReg?.catechesisYear?.replace('_', ' ')}</div></div></div><div className="flex items-baseline gap-2 py-1"><span className="whitespace-nowrap font-bold shrink-0 tracking-wide">en concepto de:</span><div className="flex-1 border-b border-dotted border-slate-400 pb-1 px-2 text-sm text-slate-700 font-medium italic leading-relaxed">Saldo Pendiente: {((selectedReg?.registrationCost || 0) - (selectedReg?.amountPaid || 0)).toLocaleString('es-PY')} Gs.</div></div></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-10"><div className="flex flex-col justify-end space-y-3"><p className="text-sm italic font-medium">Asunción, {localDate.day} de {localDate.month} de {localDate.year}</p><div className="flex flex-col items-start pt-4"><div className="w-48 border-t border-slate-900"></div><p className="text-[8px] font-bold uppercase mt-1 tracking-widest">(Firma y aclaración)</p></div></div><div className="flex flex-col items-center md:items-end gap-3"><div className="p-1.5 border border-slate-900 rounded-lg bg-white shadow-sm"><QRCodeCanvas value={`RECIBO-NSPS-${selectedReg?.id}-${paymentAmount}-${selectedReg?.receiptNumber}`} size={80} level="H" /></div><div className="text-right"><p className="text-[8px] font-black uppercase text-primary tracking-widest leading-none">Firma Digitalizada</p><p className="text-xs font-bold text-slate-900 uppercase mt-1">{selectedReg?.validatedBy || 'Secretaría del Santuario'}</p><p className="text-[8px] text-slate-500 font-bold uppercase">{profile?.role || 'Personal Institucional'}</p></div></div></div>
            </div>
          </div>
          <DialogFooter className="p-4 bg-slate-100 border-t flex flex-row gap-2"><Button variant="outline" className="flex-1 rounded-xl h-12 font-bold" onClick={() => setIsReceiptOpen(false)}>Cerrar</Button><Button className="flex-1 gap-2 rounded-xl bg-blue-600 text-white h-12 font-bold shadow-lg" onClick={handleDownloadImage} disabled={isGeneratingPDF}><ImageIcon className="h-4 w-4" /> IMAGEN</Button><Button className="flex-1 gap-2 rounded-xl bg-green-600 text-white h-12 font-bold shadow-lg" onClick={handleShareWhatsApp}><MessageCircle className="h-4 w-4" /> WHATSAPP</Button><Button className="flex-1 gap-2 rounded-xl bg-slate-900 text-white h-12 font-bold shadow-lg" onClick={handleDownloadPDF} disabled={isGeneratingPDF}>{isGeneratingPDF ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />} PDF</Button></DialogFooter>
        </DialogContent>
      </Dialog>
